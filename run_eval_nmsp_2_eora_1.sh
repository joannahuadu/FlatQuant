...
nohup env CUDA_VISIBLE_DEVICES=1 python main_comp.py \
  --model "$MODEL_PATH" \
  --w_bits 4 \
  --a_bits 4 \
  --gptq \
  ...
  --eigen_compensation \
  --eigen_dataset winogrande \
  --eigen_nsamples 64 \
  --eigen_r 128 \
  >> "$LOG_DIR/eval_w4a4_NMSP_2_skip_eora_wg_0204.log" 2>&1 &
...
nohup env CUDA_VISIBLE_DEVICES=1 python main_comp.py \
  ...
  --eigen_nsamples 64 \
  --eigen_r 128 \
  >> "$LOG_DIR/eval_w8a8_NMSP_2_skip_eora_wg_0204.log" 2>&1 &